...
class RoleEditor{
	...
	[MenuItem("NEditor/RoleEditor/BuildRoleBundle")]
	public static void BuildRoleBundle()
	{
		...
		if (!Directory.Exists(ABOutputPath)){
			Log.i ("RoleEditor","CreateAllRoleAB","输出路径"+ABOutputPath+"不存在，创建输出路径");
			Directory.CreateDirectory(ABOutputPath);
		}

		string roleResPath = EditorHelper.EDITOR_ROLE_RES_PATH;					//RoleEditor资源根目录
		...
		for (int i = 0; i < _roleFolderURLs.Length; i++){
			...
			Log.i ("RoleEditor","CreateAllRoleAB","开始打包骨骼部位，当前角色文件夹 _folderURLS["+i+"]->"+_roleFolderURLs[i]);
			...
			Log.i ("RoleEditor","CreateAllRoleAB","开始打包材质，当前角色文件夹_folderURLS["+i+"]->"+_roleFolderURLs[i]);
			GeneratePartMatAB(_matFolderUrl);
			...
			string _tempAnimClipFolderURL = EditorHelper.EDITOR_TEMP_ASSET_PATH+"/"+_roleName+"/AnimationClip/";
			...
			for (int j = 0; j < _tempAnimClipGroupURLs.Length; j++){
				...
				ab.assetBundleName = _roleName + "_anim" +"_" + _animGroupName + ABSuffixName;
		...
		BuildPipeline.BuildAssetBundles (ABOutputPath, RoleBundleBuildList.ToArray(), BuildAssetBundleOptions.CollectDependencies, BuildTarget.StandaloneWindows);
	...
	static void GenerateBoneAB(GameObject go)
	{
		...
		string _tempBoneFolderPath = EditorHelper.ChangeToRelativePath (EditorHelper.EDITOR_TEMP_ASSET_PATH) + "/"+go.name.ToLower()+"/";
	...
	static void GeneratePartMatAB(string matFolderUrl)
	{
		...
		string _tempMatNameWithoutExt = "";
		for(int i=0;i<matURLs.Length;i++)
		{
			_tempMatNameWithoutExt = EditorHelper.GetFileNameFromPath (matURLs [i], true);

			string path = ABOutputPath + _tempMatNameWithoutExt + ABSuffixName;//材质名一般格式为  角色名_部位名_模型名_材质名.mat  如果不包含这些就比较难区分是属于那个角色哪个部位

			EditorHelper.DeleteFileIfExists(path);	

			AssetBundleBuild ab = new AssetBundleBuild();
			ab.assetBundleName = _tempMatNameWithoutExt + ABSuffixName;
			string[] assetUrls = new string[1];
			string ss = EditorHelper.ChangeToRelativePath(matURLs[i]);
			assetUrls [0] = ss;
			ab.assetNames = assetUrls;
			RoleBundleBuildList.Add(ab);
	...
	static void GeneratePartAB(GameObject go){
		...
		foreach (SkinnedMeshRenderer smr in go.GetComponentsInChildren<SkinnedMeshRenderer>(true)){
			...
			//这里新建一些临时文件夹，打包完再清理掉
			string tempPartFolderPath =  EditorHelper.ChangeToRelativePath( EditorHelper.EDITOR_TEMP_ASSET_PATH)+"/" +_goName+"/"  + _goName + "_" + smr.name.ToLower()+"/";
	...
	static void AnimSeparate(string animURL){
		...
		string _outputAnimFolderPath = EditorHelper.EDITOR_TEMP_ASSET_PATH+"/"+_roleName+"/AnimationClip/"+_animGroupName;//输出AnimClip的目录
	...
	static void CleanTempAsset()
	{
		...
		//清理临时文件夹
		EditorHelper.DeleteFolder(EditorHelper.EDITOR_TEMP_ASSET_PATH);
		EditorHelper.CreateFolder (EditorHelper.EDITOR_TEMP_ASSET_PATH);
	}
